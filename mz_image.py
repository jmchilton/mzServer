...
dpi = 100.0 # matplotlib's default printing dpi is 100.

# matplotlib has a convoluted way of deciding image size.
# rather than try to change the plot size, I change the
# DPI to be used for printing, which will scale the output
# properly. This at least has the nice property of scaling
# images uniformly--ie without distortion
def set_size(new_width, new_height):
    global dpi
    dpi = int((new_width+new_height)/(14.0) * 100.0)
...
# lighter version of the singly-charged color
ion_color_dict = {  'b': '#0000FF',   'b++': '#9696FF',
                    'y': '#FF0000',   'y++': '#FF9696',
                    'c': '#008000',   'c++': '#32CD32',
                  'z+1': '#FF00FF', 'z+1++': '#FF66FF'}
...
ETDTRAP_ion_dict = {  'c': (0.93, '_'),   'c++': (0.95, '_'),
                      'y': (0.87, '_'),   'y++': (0.85, '_'),
                    'z+1': (0.83, '~'), 'z+1++': (0.81, '~')}
...
def get_color(labels):
    ...
    lbl_re = re.compile(r'(.+?)[\*0]?\(\d+\).*?([\+\-]{2})?(?: \[\d+\.\d+\])?$')
    lbl_set = list(set(lbl_re.match(lbl).groups() for lbl in labels))
    if len(lbl_set) == 1:
        i = lbl_set[0][0] + (lbl_set[0][1] or '')

        if i in ion_color_dict:
            return ion_color_dict[i]
        else:
            return '#FFA500' # orange for unknown ion label
    else:
        return '#CDCD00' # yellow for multiple ion types--fairly uncommon


def make_ms1_im(save_file, mz, xy, scan_mode, pm_scanDot, fmt='PNG', acquisition_time=None):
    '''Plots an MS scan, centered on the target mz'''

    fig = Figure()

    _make_ms1(fig, mz, xy, scan_mode, pm_scanDot, acquisition_time=acquisition_time)

    FigureCanvasAgg(fig).print_figure(save_file, dpi=dpi, format=fmt)


def make_xic_im(save_file, mz, xic_time, xic_int, scan_dot, bin_times, bin_ints, fmt='PNG'):
    '''Plots a XIC, with labels for the primary MSMS scan and any neighboring scans
    in the same time and mz area'''

    fig = Figure()

    _make_xic(fig, mz, xic_time, xic_int, scan_dot, bin_times, bin_ints)

    FigureCanvasAgg(fig).print_figure(save_file, dpi=dpi, format=fmt)


def make_ms2_im(save_file, scan, scan_mode, peptide, labels=None, ion_list=('b', 'y'),
                charge='', score='', fmt='PNG', label_top=50, filter_labels=True,
                acquisition_time=None): #, pretty_print=True):
    ...
    _make_ms2(fig, scan, scan_mode, peptide, labels,
              ...
              filter_labels, acquisition_time=acquisition_time) #, pretty_print)

    FigureCanvasAgg(fig).print_figure(save_file, dpi=dpi, format=fmt)


def make_mascot_ms2_im(save_file, web_img, peptide, charge, score, ion_info=None,
                       phospho=None, oxid=None, instrument='ESI-TRAP', fmt='PNG'):
...
def make_mirror_im(save_file, scan1, scan2, title="Mirror Image", fmt='PNG'):
...
def _make_ms1(fig, mz, xy, scan_mode, pm_scanDot, acquisition_time=None):
    '''Plots an MS scan, centered on the target mz.
    ...
    axes.plot([i[0] for i in plot_xy], [i[1] for i in plot_xy], c='k')
    axes.axvline(x=pm_scanDot[0], ymin=0, ymax=1, ls='--', c='b')

    if isinstance(acquisition_time, float):
        acquisition_time = '%.2f' % acquisition_time

    if isinstance(mz, float):
        mz = '%.3f' % mz

    axes.set_title(''.join(('Acquisition ' if (acquisition_time and not mz) else '',
                            'Time: %s min' % acquisition_time if acquisition_time else '',
                            '    ' if (acquisition_time and mz) else '',
                            '%s m/z' % mz if mz else '')))

    axes.set_xlabel('m/z')
    axes.set_xlim((pm_scanDot[0] - 2.1, pm_scanDot[0] + 2.1))
...
def _make_xic(fig, mz, xic_time, xic_int, scan_dot, bin_times, bin_ints):
    '''Plots a XIC, with labels for the primary MSMS scan and any neighboring scans
    in the same time and mz area.
...
def _make_ms2(fig, scan, scan_mode, peptide, labels=None, ion_list=('b', 'y'),
              charge='', score='', label_top=50, filter_labels=True,
              pretty_print=False, acquisition_time=None):
    ...
    if peptide:
        ...
        if labels is None: # create theoretical fragments and do some labeling
            ...
            labels = generate_labels(sorted(scan, key=lambda mi: mi[1], reverse=True)[:label_top],
                                     peptide, ion_list) # default tolerance is 0.6 Da
    ...
    axes.stem([x1 for x1,y1 in scan if y1 > 0],
    ...
    # set the title if charge and/or score provided
    axes.set_title(''.join(('Acquisition ' if (acquisition_time and not (charge or score)) else '',
                            'Time: %s min' % acquisition_time if acquisition_time else '',
                            '    ' if (acquisition_time and (charge or score)) else '',
                            'Charge: %s' % charge if charge else '',
